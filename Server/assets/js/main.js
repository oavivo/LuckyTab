//Support function to handle get params
function qs(key) {
...
$(document).ready(function(){
	var contactUsValue = qs("contactUs");
	if(contactUsValue != null){		
		$('#contactForm').modal('show');
		var sel = document.getElementById('Subject');
		var val = contactUsValue;
		for(var i = 0, j = sel.options.length; i < j; ++i) {			
			if($(sel.options[i]).attr('value') === val) {
			   sel.selectedIndex = i;
			   break;
			}
		}
		if(val == 4 && qs("urlToReport") != null){
			$("#Message").val("Item to report: "+qs("urlToReport"));
		}
	}
